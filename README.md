...
The study runs in three sequential stages (`comments.txt` §4):

- **Stage 1** — vanilla tanh-RNN training-regime map: supervision
...
- **Stage 2** — the 15-cell, 5-arm (S, M, P, T, D) bio-plausibility ablation
  battery on the GRU substrate, run by `run_grid.py`. This is the study's
  main event and the one `make run-grid` drives.
...
specification live in `comments.txt` (the current, authoritative build spec;
`../protocol_v4_master_prompt.md` is the superseded original design doc).
This README covers only setup and day-to-day usage.
...
after each invocation. Stage 1's `--max-steps` has no default: it is set
...
| `comments.txt` | The current, authoritative build spec: study structure, phases, gates, decisions-already-made |
| `PHASE_LOG.md` | Per-phase changelog: what changed, the acceptance output, and any honest negatives |
| `preregistration.md` | Frozen hypotheses, predictions, and the analysis lock; amendments appended, never rewritten |
