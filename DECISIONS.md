...
- **2026-07-08: Rutishauser data-preprocessing verification (user-requested) found two real gaps in the neural adapter, both fixed.** A research pass cross-checking `dandi_nwb.py` against the datasets' own NWB structure and source papers (Kyzar et al. 2024, Daume et al.) confirmed WM-session identification, column mapping, outcome-field semantics, and epoch timing windows are correct for Tier A (000469+000673) -- the maintenance window (1.5s) has real headroom against an empirically-measured ~2.5-2.7s median true delay duration. Two gaps found and fixed:
  1. **Unit QC never used the isolation-distance/SNR metrics the NWB releases actually carry** (`units/waveforms_isolation_distance`, matching Kyzar et al. 2024's own reported spike-sorting quality metrics) -- only bare firing rate gated inclusion. Added `min_isolation_distance` (config default 20.0, a standard single-unit isolation threshold in this literature, Harris et al. 2001/Schmitzer-Torbert et al. 2005) to `DandiSternbergTierA`/`_apply_firing_qc`: a unit fails if its isolation distance is NaN (metric uncomputable) or below threshold, but the criterion is skipped entirely for a dataset that doesn't carry the field at all (not conflating "absent metric" with "failed metric"). Verified against real mounted data: reduces the sampled unit set by ~35% (187 -> 119 units across 6 sessions), confirming the filter is doing real work, not a no-op.
  2. **Tier B (001187) could not load at all**: its Sternberg trials live under `intervals/WM_trials`, not the top-level `intervals/trials` 000469/000673 use (001187 also has an unrelated `intervals/LTM_trials` New/Old-recognition task that must not be picked up as WM data). `find_wm_sessions`/`_load_session` now check both candidate group names (`TRIALS_GROUP_CANDIDATES`); 001187's `WM_trials` schema matches 000673's exactly, so it reuses that `COLUMN_MAPS` entry. Verified against real data: 001187 now loads (3 sessions/23 units checked, contract-valid). Tier B is loadable but still not wired into any analysis path -- that remains future scope, not attempted here.
  Both fixes are regression-tested (`tests/test_dandi_adapter.py::test_isolation_distance_qc_excludes_poorly_isolated_units`, `::test_tier_b_001187_loads_via_wm_trials_group`).
- **2026-07-08: Phase C (full-tier grid launch) and Phase D (aggregation/RUN_REPORT.md) deferred at user's request.** Phase A (code) and Phase B (smoke-tier pipeline validation across all 15 battery cells + a dev-tier M11111 topo+dale training sanity check) are complete; the actual multi-day full-tier launch is intentionally NOT started this session. See RESPONSES.md's "Next steps" section for the concrete resume commands.
- **2026-07-08: cleared stale results superseded by the 5-arm pivot + the isolation-distance QC change.** Deleted: (a) all 48 manifest rows/checkpoints from the old 3-bit S/M/P factorial (M000-M111) and its old-anchor supplementary arms (M000_2x/_l1/_dropout/_idcatch, M111_energy/_noise/_pbwm/_idcatch) -- these cells no longer exist in the 15-cell battery design; (b) all 15 of today's smoke-tier (100-step) manifest rows/checkpoints for the new battery cells and the dev-tier M11111 sanity-check run -- these are Phase B validation artifacts, not real results, and would otherwise cause a future `run_grid.py --tier full` to skip those cells thinking they're already complete (manifest resume logic checks only `status=="completed"`, not tier); (c) every derived analysis CSV/jsonl (`alignment_results.csv`, `network_properties.jsonl`, `dynamics_persistence.csv`, etc.) and `RUN_REPORT.md`, all computed from the now-deleted old checkpoints. PRESERVED: the 8 local-learning (M**L) checkpoints/manifest rows (comments.txt explicitly says do not retrain -- that sub-study is separate and already complete) and `results/feat_cache` (expensive-to-regenerate stimulus features, dataset-derived not model-derived).
